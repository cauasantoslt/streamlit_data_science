# 2_Modelagem_Preditiva.py (CORRIGIDO)
...
df_filtered = df[
    (df['Fertilizante'].isin(selected_fertilizer)) &  # <-- CORRIGIDO AQUI
    (df['Tipo de Solo'].isin(selected_soil)) &        # <-- CORRIGIDO AQUI
    (df['Temperatura'] >= temp_min) & (df['Temperatura'] <= temp_max) & # <-- CORRIGIDO AQUI
    (df['Precipitação'] >= precip_min) & (df['Precipitação'] <= precip_max) & # <-- CORRIGIDO AQUI
    (df['Umidade'] >= umid_min) & (df['Umidade'] <= umid_max) # <-- CORRIGIDO AQUI
]
